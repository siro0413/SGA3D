...
#pragma endregion
	...
	if (brushDesc.Type > 0);
	{
		...
		{
			if (Mouse::Get()->Down(0))
				SlopeHeight(brushDesc.Location, brushDesc.Location2, brushDesc.Range);
		}
		else
			brushDesc.Location = GetPickedPosition();

	}
	ImGui::LabelText("pos", "%0.2f,%0.2f", brushDesc.Location.y, brushDesc.Location2.y);
	ImGui::LabelText("count", "%d", count);
...
void Terrain::SlopeHeight(Vector3 & pos1, Vector3 & pos2, UINT range)
{
	brushDesc.Location = pos1;
	brushDesc.Location2 = pos2;
	if (count == 0)
	{
		pos1 = GetPickedPosition();
		count++;
	}
	else
	{
		pos2 = GetPickedPosition();
		float tx = pos1.x - pos2.x;
		float ty = pos1.y - pos2.y;
		float tz = pos1.z - pos2.z;
		float tdist = sqrt(tx * tx + tz * tz);

		float lean = tz / tx;
		float StartX = tx > 0 ? pos2.x : pos1.x;
		float zvalue = tx > 0 ? pos2.z : pos1.z;
		float EndX = tx > 0 ? pos1.x : pos2.x;
		float leanpos = tx > 0 ? StartX : EndX;
		for (int x = StartX; x <= EndX; x++)
		{
			for (size_t s = 0; s < range * 2; s++)
			{
				for (int i = 0; i < range * 2; i++)
				{
					int z = (lean * (x - StartX) + zvalue);
					UINT index = width * ((UINT)z + s) + ((UINT)x + i);
					float dx = leanpos - vertices[index].Position.x;
				
					float dz = (lean * (leanpos - StartX) + zvalue) - vertices[index].Position.z;
					float dist = sqrt(dx * dx + dz * dz) / tdist;

					vertices[index].Position.y = ty * dist;
				}
			}
		}
...
D3D11_BOX Terrain::RectBox(Vector3& pos, UINT range)
{
	D3D11_BOX rect;
